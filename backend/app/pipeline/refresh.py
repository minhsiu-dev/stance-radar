...
from app.models import (
    Channel, Mention, Stance, Video, VideoStance, VideoStatus, utcnow,
)
...
class RefreshRunner:
    ...
    async def start(self) -> tuple[int, bool]:
        ...
        async with self._start_lock:
            async with self._deps.sessionmaker() as session:
                ...
                job_id = job.id
            if created:
                self.current_task = asyncio.create_task(self._run_safely(job_id))
        return job_id, created

    async def _run_safely(self, job_id: int) -> None:
        try:
            ...
            await jobs.finish_job(self._deps.sessionmaker, job_id, error=f"更新失敗:{exc}")
    ...
    async def _run(self, job_id: int) -> None:
        ...
        for i, channel in enumerate(channels):
            ...
            await self._ingest_channel_videos(channel)

        async with deps.sessionmaker() as session:
            pending = list((await session.execute(
                select(Video.id)
                .where(Video.status.in_([VideoStatus.pending, VideoStatus.failed]))
        ...
        await jobs.update_progress(deps.sessionmaker, job_id, {
            ...
            "videos_done": 0, "videos_total": total,
        })
        done = 0
        ...
        async def process(video_id: str) -> None:
            ...
            async with progress_lock:
                ...
                await jobs.update_progress(deps.sessionmaker, job_id, {
                    "stage": "analyzing",
                    "channels_done": total_channels, "channels_total": total_channels,
                    "videos_done": done, "videos_total": total,
                })

        await asyncio.gather(*(process(video_id) for video_id in pending))

    async def _ingest_channel_videos(self, channel: Channel) -> None:
        deps = self._deps
        async with deps.sessionmaker() as session:
